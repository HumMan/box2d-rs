...
pub(crate) fn dump<D: UserDataType>(this: &B2world<D>) {
	b2_assert(this.is_locked() == false);
	if this.is_locked() {
		panic!();
	}

	// b2Log("let g = B2vec2::new(%.15lef, %.15lef);\n", m_gravity.x, m_gravity.y);
	// b2Log("m_world->set_gravity(g);\n");

	// b2Log("b2_body** bodies = (b2_body**)b2Alloc(%d * sizeof(b2_body*));\n", m_body_count);
	// b2Log("B2joint** joints = (B2joint**)b2Alloc(%d * sizeof(B2joint*));\n", m_joint_count);
	// i32 i = 0;
	// for (b2_body* b = m_body_list; b; b = b->m_next)
	// {
	// 	b->m_island_index = i;
	// 	b->dump();
	// 	++i;
	...
	// i = 0;
	// for (B2joint* j = m_joint_list; j; j = j->m_next)
	// {
	// 	j->m_index = i;
	// 	++i;
	// }
