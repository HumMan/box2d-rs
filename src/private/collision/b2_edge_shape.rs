...
pub fn b2_set_two_sided(this: &mut B2edgeShape, v1: B2vec2, v2: B2vec2) {
	this.m_vertex1 = v1;
	this.m_vertex2 = v2;
	this.m_one_sided = false;
}

pub fn b2_shape_dyn_trait_clone(this: &B2edgeShape) -> Box<dyn B2shapeDynTrait> {
	return Box::new(B2edgeShape::clone(&this));
}

pub fn b2_shape_dyn_trait_get_child_count(_this: &B2edgeShape) -> usize {
	return 1;
}

pub fn b2_shape_dyn_trait_test_point(_this: &B2edgeShape, xf: B2Transform, p: B2vec2) -> bool {
	b2_not_used(xf);
	b2_not_used(p);
	return false;
}
...
pub fn b2_shape_dyn_trait_ray_cast(
	...
	if this.m_one_sided && numerator > 0.0 {
		return false;
	}
...
pub fn b2_shape_dyn_trait_compute_mass(this: &B2edgeShape, mass_data: &mut B2massData, density: f32) {
	b2_not_used(density);
