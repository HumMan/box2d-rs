...
pub(crate) struct MobileBalanced<D: UserDataType> {
	base: TestBasePtr<D>,
...
impl<D: UserDataType> MobileBalanced<D> {
	...
	fn add_node(
		...
		shape.set_as_box_angle(offset, 0.25 * a, B2vec2::new(0.0, -a), 0.0);
		B2body::create_fixture_by_shape(body.clone(), Rc::new(RefCell::new(shape)), density);
...
impl<D: UserDataType, F: Facade> TestDyn<D, F> for MobileBalanced<D> {
